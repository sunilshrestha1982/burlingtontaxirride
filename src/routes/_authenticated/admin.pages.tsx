...
import { legacyContentBlocks, type LocationPageContent, type PageDraft } from "@/lib/page-content";
    ...
    {pages.length > 0 && <div className="grid gap-6 xl:grid-cols-[310px_minmax(0,1fr)]">
      ...
      {current && draft && <main className="min-w-0 space-y-6">
        <form className="space-y-7" onSubmit={(event) => { event.preventDefault(); saveMutation.mutate({ ...draft, id: current.id }); }}>
          ...
          {current.page_type === "location" && <EditorSection title="Long page sections" subtitle="Build the complete taxi page. Changes appear immediately in the preview."><PageBlockEditor blocks={legacyContentBlocks(draft.content, draft.destination_name || draft.nav_label)} onChange={(blocks) => setDraft({ ...draft, content: { ...draft.content, blocks } })}/></EditorSection>}
        ...
        {preview && <div className="overflow-hidden rounded-md border border-gold/30"><div className="sticky top-0 z-10 flex items-center justify-between border-b border-border bg-surface px-4 py-2 text-xs uppercase tracking-widest text-gold"><span>Draft preview — not published</span><span>Live full page</span></div><div className="max-h-[78vh] overflow-y-auto"><div className="pointer-events-none"><PageHero eyebrow={draft.eyebrow} title={draft.hero_title || draft.nav_label} highlight={draft.hero_highlight} description={draft.hero_description} backgroundImage={draft.hero_image || undefined}/></div>{current.page_type === "location" && <TaxiPageBlocks preview blocks={legacyContentBlocks(draft.content, draft.destination_name || draft.nav_label)}/>}<div className="space-y-1 border-t border-border p-5"><p className="text-[10px] uppercase tracking-widest text-muted-foreground">Search result preview</p><p className="text-lg text-gold">{draft.meta_title || draft.nav_label}</p><p className="text-xs text-emerald-500">burlingtonvttaxiride.com{draft.slug}</p><p className="text-sm text-muted-foreground">{draft.meta_description}</p></div></div></div>}
      </main>}
    </div>}
  </div>;
}
